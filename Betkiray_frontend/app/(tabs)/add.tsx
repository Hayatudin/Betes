...
export default function AddScreen() {
  ...
  const StepHeader = ({ step }: { step: number }) => (
    <View style={[styles.headerContainer, { backgroundColor: colors.background }]}>
      <TouchableOpacity
        style={[styles.closeBtn, { backgroundColor: isDark ? colors.secondary : '#fff' }]}
        onPress={() => {
          if (step > 1) {
            goBack();
          } else {
      ...
      <View style={styles.progressContainer}>
        <Text style={[styles.stepIndicator, { color: colors.muted }]}>Step {step} of 4</Text>
        <View style={styles.progressRow}>
          {[1, 2, 3, 4].map((i) => (
            <View key={i} style={[styles.progressSegment, step >= i ? { backgroundColor: colors.text } : { backgroundColor: isDark ? colors.border : '#E0E0E0' }]} />
          ))}
        </View>
      </View>
      <View style={{ width: 40 }} />
  ...
  return (
    <KeyboardAvoidingView style={[styles.keyboardContainer, { backgroundColor: colors.background }]} behavior={Platform.OS === "ios" ? "padding" : "height"}>
      ...
      <ScrollView style={[styles.container, { backgroundColor: colors.background }]} showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        {step === 1 && (
          <View style={styles.stepBody}>
            <Text style={[styles.mainTitle, { color: colors.text }]}>Upload Photos of Your Property</Text>

            <View style={styles.photoGrid}>
              <TouchableOpacity
                style={[styles.photoMainSlot, photos[0] ? styles.slotFilled : { backgroundColor: isDark ? colors.secondary : '#FAFAFA', borderColor: isDark ? colors.border : '#D0D0D0' }]}
                onPress={pickImage}
              >
                {photos[0] ? (
                  <>
                  ...
                  <View style={styles.placeholderContent}>
                    ...
                    <View style={[styles.plusBadge, { backgroundColor: colors.text }]}><Ionicons name="add" size={10} color={colors.background} /></View>
                  </View>
                )}
              </TouchableOpacity>
              ...
              <View style={styles.photoColRight}>
                {[1, 2].map((i) => (
                  <TouchableOpacity
                    key={i}
                    style={[styles.photoSmallSlot, photos[i] ? styles.slotFilled : { backgroundColor: isDark ? colors.secondary : '#FAFAFA', borderColor: isDark ? colors.border : '#D0D0D0' }]}
                    onPress={pickImage}
                  >
                    {photos[i] ? (
                      <>
                      ...
                      <View style={styles.placeholderContent}>
                        ...
                        <View style={[styles.plusBadgeSmall, { backgroundColor: colors.text }]}><Ionicons name="add" size={8} color={colors.background} /></View>
                      </View>
                    )}
                  </TouchableOpacity>
                ))}
            ...
            <TouchableOpacity style={[styles.videoRow, { backgroundColor: isDark ? colors.secondary : '#fff', borderColor: isDark ? colors.border : '#F0F0F0' }]}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                <Ionicons name="videocam" size={24} color={colors.muted} />
                <Text style={[styles.videoText, { color: colors.text }]}>Add Video (Optional)</Text>
              </View>
        ...
        {step === 2 && (
          <View style={styles.stepContainer}>
            ...
            <View style={[styles.cardContainer, { backgroundColor: isDark ? colors.secondary : '#fff' }]}>
              ...
              />
              <View style={[styles.inputUnderline, { backgroundColor: isDark ? colors.border : '#E5E5EA' }]} />
            </View>

            {/* Description */}
            <View style={[styles.cardContainer, { marginTop: 12, height: 140, backgroundColor: isDark ? colors.secondary : '#fff' }]}>
              ...
              <TextInput
                ...
                style={[styles.inputClean, { height: '80%', textAlignVertical: 'top', color: colors.text }]}
                multiline
                maxLength={500}
              />
            </View>
            ...
            {/* Voice Input */}
            <TouchableOpacity style={[styles.voiceButton, { backgroundColor: isDark ? colors.secondary : '#fff' }]} onPress={isRecording ? stopRecording : startRecording}>
              {isRecording ? (
                ...
                <View style={styles.voiceContent}>
                  ...
                  <Text style={[styles.voiceText, { color: colors.muted }]}>Tap to add description by voice</Text>
                  <Ionicons name="cellular-outline" size={20} color={colors.border} />
                </View>
              )}
            </TouchableOpacity>

            {/* Phone Number */}
            <View style={[styles.cardContainer, { marginBottom: 12, backgroundColor: isDark ? colors.secondary : '#fff' }]}>
              <Text style={styles.labelLight}>Phone Number</Text>
              <TextInput
                placeholder="09(7)...."
                placeholderTextColor={colors.muted}
                value={phone}
            ...
            {/* Area */}
            <View style={[styles.cardContainer, { marginBottom: 12, backgroundColor: isDark ? colors.secondary : '#fff' }]}>
              <Text style={styles.labelLight}>Area (m²)</Text>
              <TextInput
                placeholder="e.g. 100"
                placeholderTextColor={colors.muted}
                value={areaSqm}
            ...
            {/* Property Type */}
            <View style={[styles.cardContainer, { backgroundColor: isDark ? colors.secondary : '#fff' }]}>
              <Text style={[styles.sectionHeader, { color: colors.text }]}>Preperty Type</Text>
                ...
                {(["House", "Apartment", "Office", "MarketPlace"] as const).map((t) => (
                  <TouchableOpacity
                    ...
                    style={[
                      ...
                      type === t || (t === "MarketPlace" && type === "Retail") ? styles.typeChipActiveClean : { backgroundColor: isDark ? colors.border : '#F5F5F5' }
                    ]}
                    onPress={() => setType(t === "MarketPlace" ? "Retail" : t as any)}
                  >
                    <Text style={[
                      styles.typeChipTextClean,
                      type === t || (t === "MarketPlace" && type === "Retail") ? styles.typeChipTextActiveClean : { color: colors.text }
                    ]}>
                      {t}
                    </Text>
                  </TouchableOpacity>
            ...
            <View style={[styles.cardContainer, { backgroundColor: isDark ? colors.secondary : '#fff' }]}>
              ...
              {["House", "Apartment"].includes(type) && (
                <>
                  <View style={styles.roomRow}>
                    <Text style={[styles.roomLabel, { color: colors.text }]}>Bedrooms</Text>
                    <View style={styles.counterWrapper}>
                      <TouchableOpacity style={[styles.counterBtnSmall, { backgroundColor: isDark ? colors.border : '#F5F5F5' }]} onPress={() => setRooms(v => Math.max(1, v - 1))}>
                        <Ionicons name="remove" size={16} color={colors.text} />
                      </TouchableOpacity>
                      <Text style={[styles.counterValSmall, { color: colors.text }]}>{rooms}</Text>
                      <TouchableOpacity style={[styles.counterBtnSmall, { backgroundColor: isDark ? colors.border : '#F5F5F5' }]} onPress={() => setRooms(v => v + 1)}>
              ...
              <View style={styles.roomRow}>
                <Text style={[styles.roomLabel, { color: colors.text }]}>Bathrooms</Text>
                <View style={styles.counterWrapper}>
                  <TouchableOpacity style={[styles.counterBtnSmall, { backgroundColor: isDark ? colors.border : '#F5F5F5' }]} onPress={() => setBaths(v => Math.max(0, v - 1))}>
                    <Ionicons name="remove" size={16} color={colors.text} />
                  </TouchableOpacity>
                  <Text style={[styles.counterValSmall, { color: colors.text }]}>{baths}</Text>
                  <TouchableOpacity style={[styles.counterBtnSmall, { backgroundColor: isDark ? colors.border : '#F5F5F5' }]} onPress={() => setBaths(v => v + 1)}>
                    <Ionicons name="add" size={16} color={colors.text} />
                  </TouchableOpacity>
                </View>
              </View>
            ...
            {/* Furnished */}
            <View style={[styles.furnishedContainer, { backgroundColor: isDark ? colors.secondary : '#fff' }]}>
              <Text style={[styles.sectionHeader, { color: colors.text, marginBottom: 0 }]}>Furnished</Text>
              <CustomSwitch
                value={furnished}
        ...
        {step === 3 && (
          <View style={styles.stepContainer}>
            {/* Price Card */}
            <View style={[styles.inputCard, { backgroundColor: isDark ? colors.secondary : '#fff' }]}>
            ...
            {/* Negotiable Card */}
            <View style={[styles.toggleCard, { backgroundColor: isDark ? colors.secondary : '#fff' }]}>
              <Text style={[styles.toggleLabel, { color: colors.text }]}>Negotiable</Text>
              <CustomSwitch
                value={negotiable}
            ...
            {/* Utilities Card */}
            <View style={[styles.toggleCard, { backgroundColor: isDark ? colors.secondary : '#fff' }]}>
              <Text style={[styles.toggleLabel, { color: colors.text }]}>Include Utilities</Text>
              <CustomSwitch
        ...
        {step === 4 && (
          <View style={styles.stepContainer}>
            {/* Map View */}
            <View style={{ height: 250, borderRadius: 16, overflow: 'hidden', marginBottom: 20 }}>
              <LeafletMapView
                latitude={coords.lat}
            ...
            {/* Full Address Card */}
            <View style={[styles.addressCard, { backgroundColor: isDark ? colors.secondary : '#fff' }]}>
              <Text style={styles.cardLabel}>Full address</Text>
              <View style={styles.addressRow}>
                <View style={[styles.iconContainer, { backgroundColor: isDark ? colors.border : '#F5F5F5' }]}>
            ...
            {/* City Card */}
            <TouchableOpacity style={[styles.addressCard, { backgroundColor: isDark ? colors.secondary : '#fff' }]} onPress={() => setCityDropdownVisible(true)}>
            ...
            {/* Use Location Button */}
            <TouchableOpacity style={styles.locationBtnLight} onPress={useMyLocation} disabled={isFetchingLocation}>
              {isFetchingLocation ? <ActivityIndicator size="small" color="#000" /> : null}
              <Text style={styles.locationBtnTextLight}>Use current location (Optional)</Text>
            </TouchableOpacity>
          </View>
        )}
      </ScrollView>
      ...
      {step === 1 && (
        <View style={[styles.footerContainer, { backgroundColor: isDark ? colors.background : '#fff', borderTopColor: isDark ? colors.border : '#F0F0F0' }]}>
          <TouchableOpacity style={[styles.blackButton, { backgroundColor: isDark ? colors.text : '#000' }]} onPress={goNext}>
            <Text style={[styles.blackButtonText, { color: isDark ? colors.background : '#fff' }]}>Next</Text>
          </TouchableOpacity>
        </View>
      )}
      ...
      {step === 4 && (
        <View style={[styles.footerContainer, { backgroundColor: isDark ? colors.background : '#fff', borderTopColor: isDark ? colors.border : '#F0F0F0' }]}>
          <TouchableOpacity style={[styles.blackButton, { backgroundColor: isDark ? colors.text : '#000' }]} onPress={postProperty} disabled={isSubmitting}>
            {isSubmitting ? <ActivityIndicator size="small" color={isDark ? colors.background : '#fff'} /> : null}
            <Text style={[styles.blackButtonText, { color: isDark ? colors.background : '#fff' }]}>{isSubmitting ? "Posting..." : "Post my property"}</Text>
          </TouchableOpacity>
        </View>
      )}
      ...
      <Modal visible={cityDropdownVisible} transparent={true} animationType="fade" onRequestClose={() => setCityDropdownVisible(false)}>
        <TouchableOpacity style={styles.modalOverlay} onPress={() => setCityDropdownVisible(false)}>
          <View style={[styles.dropdownContainer, { backgroundColor: isDark ? colors.secondary : '#fff' }]}>{cities.map(c => (<TouchableOpacity key={c} style={[styles.dropdownItem, { borderBottomColor: isDark ? colors.border : '#F0F0F0' }]} onPress={() => { setCity(c); if (c !== 'Addis Ababa') { setSubCity(null); } setCityDropdownVisible(false); }}><Text style={[styles.dropdownItemText, { color: colors.text }]}>{c}</Text></TouchableOpacity>))}</View>
        </TouchableOpacity>
      </Modal>
      ...
      <Modal visible={subCityDropdownVisible} transparent={true} animationType="fade" onRequestClose={() => setSubCityDropdownVisible(false)}>
        <TouchableOpacity style={styles.modalOverlay} onPress={() => setSubCityDropdownVisible(false)}>
          <View style={[styles.dropdownContainer, { backgroundColor: isDark ? colors.secondary : '#fff' }]}><ScrollView>{addisAbabaSubCities.map(sc => (<TouchableOpacity key={sc} style={[styles.dropdownItem, { borderBottomColor: isDark ? colors.border : '#F0F0F0' }]} onPress={() => { setSubCity(sc); setSubCityDropdownVisible(false); }}><Text style={[styles.dropdownItemText, { color: colors.text }]}>{sc}</Text></TouchableOpacity>))}</ScrollView></View>
        </TouchableOpacity>
      </Modal>
    </KeyboardAvoidingView>
  );
...
const styles = StyleSheet.create({
  ...
  addressCard: { backgroundColor: '#fff', borderRadius: 16, padding: 16, marginBottom: 12, shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.02, shadowRadius: 2, elevation: 1 },
  addressRow: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  iconContainer: { width: 32, height: 32, borderRadius: 16, backgroundColor: '#F5F5F5', alignItems: 'center', justifyContent: 'center' },
  addressInput: { flex: 1, fontSize: 16, color: '#000', padding: 0 },
  addressText: { fontSize: 16, color: '#000', fontWeight: '500' },
  locationBtnLight: { backgroundColor: '#fff', borderRadius: 16, paddingVertical: 16, alignItems: 'center', justifyContent: 'center', marginTop: 10, shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.02, shadowRadius: 2, elevation: 1, flexDirection: 'row', gap: 8 },
  locationBtnTextLight: { fontSize: 16, fontWeight: '600', color: '#000' },
});